...
| `/projects/{projectAbbr}/objects/{id}/datastreams[?tags\|mimetype]` | Listet Metadaten zu allen Datenströmen, die zu einem Objekt gehören. Filtern via Parameter. Erlaubt sind Filter nach `tags` oder `mimetype` | `GET` |
| `/projects/p{rojectAbbr}/objects/{id}/datastream[?tags=<string>]` | Metadaten des Hauptdatenstrom des Objekts (ohne Query-Parameter) - falls kein Hauptdatenstrom definiert ist dann Fehler. Mit Query-Parameter `tags` wird entweder genau der Inhalt eines Datenstroms zurückgegeben (z.B. Image) oder ein Fehler, falls `tags` auf mehrere Datenströme passt | `GET` |
